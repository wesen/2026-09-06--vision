...
import subprocess
import time
root=Path(__file__).resolve().parents[1];out=Path('output/verifier-v1/image-gate');out.mkdir(parents=True,exist_ok=True)
...
for pin in json.loads((root/'various/candidate-pins.json').read_text()):
