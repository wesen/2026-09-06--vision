...
protocol=json.loads((out/'protocol.json').read_text());manifest=json.loads((source/'manifest.json').read_text());summary=[];raw=[]
...
for m in manifest:
 ...
 for split in ['development','test']:
  ...
  for c in protocol['cases']:
   ...
   row=dict(model=name,split=split,sample_id=c['sample_id'],expected=c['expected'],predicted=predicted,correct=predicted==c['expected'],result=d)
...
draw.text((25,20),'Frozen reviewed frame evaluation | strict valid-answer correctness',font=font,fill='black')
