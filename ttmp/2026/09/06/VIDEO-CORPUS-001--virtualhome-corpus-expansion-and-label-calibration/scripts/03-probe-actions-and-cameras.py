...
OUT=Path('output/virtualhome-corpus/diversity-probes');TICKET=Path(__file__).resolve().parent.parent
cfg=json.loads(Path('configs/virtualhome-diversity-v2.json').read_text())
comm=UnityCommunication(port='18082',timeout_wait=180)
results=[]
for scene in cfg['scenes']:
 for scenario in scene['scenarios']:
  ...
  except Exception as e:item.update(success=False,error=repr(e))
  results.append(item);print(json.dumps(item),flush=True)
  (TICKET/'various/action-camera-probes.json').write_text(json.dumps(results,indent=2)+'\n')
