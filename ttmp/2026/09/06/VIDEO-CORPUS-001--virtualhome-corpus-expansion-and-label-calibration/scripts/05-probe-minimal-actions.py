...
OUT=Path('output/virtualhome-corpus/diversity-probes-r3');OUT.mkdir(exist_ok=True);TICKET=Path(__file__).resolve().parent.parent
...
for scene in cfg['scenes']:
 for scenario in scene['scenarios']:
  ...
  try:
   ...
   (TICKET/'various/action-camera-probes-r3.json').write_text(json.dumps(results,indent=2)+'\n')
