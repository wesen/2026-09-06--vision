...
OUT=Path('output/virtualhome-corpus/diversity-probes-r4');OUT.mkdir(exist_ok=True);TICKET=Path(__file__).resolve().parent.parent
...
for scene in cfg['scenes']:
 for scenario in scene['scenarios']:
  if (scene['scene_index'],scenario['family']) not in ((1,'switch'),(2,'posture')):continue
  ...
  try:
   ...
   (TICKET/'various/action-camera-probes-r4.json').write_text(json.dumps(results,indent=2)+'\n')
