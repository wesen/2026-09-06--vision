...
def create_app(db_path, manifest, embedder):
    ...
    r = Registry(db_path)
    try:
        episodes = {e["episode_id"]: e for e in r.episodes()}
    finally:
        r.close()
    ...
    def search(request: SearchRequest):
        ...
        if (
            request.index_id is not None
            and request.index_id != index.manifest["index_id"]
        ):
        ...
        return SearchResponse(
            ...
            feature_space_id=embedder.space.id,
            mode=embedder.space.mode,
            elapsed_seconds=time.perf_counter() - started,
            hits=[
                SearchHit(
                    **{k: v for k, v in h.items() if k in SearchHit.model_fields},
                    feature_space_id=embedder.space.id,
                    video_url=f"/v1/episodes/{h['episode_id']}/video",
                )
                for h in hits
            ],
        )
    ...
    def video(episode_id: str):
        ...
        try:
            valid = file_hash(ep["video"]) == ep["video_sha256"]
        except FileNotFoundError:
            raise HTTPException(410, "source video missing")
        if not valid:
            raise HTTPException(409, "source video changed since ingest")
