...
class QwenEmbedder:
    def __init__(self, path):
        ...
        self.space = FeatureSpace(
            ...
            runtime=tuple(
                ...
                for name in ("mlx", "mlx-metal", "numpy", "pillow", "av")
            ),
        )
    ...
    def image(self, image):
        ...
        image = image.convert("RGB").resize(
            self.space.image_size, Image.Resampling.BICUBIC
        )
        return self._encode([{"type": "image"}], images=[image])

    def video(self, *_args, **_kwargs):
        raise NotImplementedError(
            "mlx-vlm 0.6.17 qwen3_vl_embedding drops pixel_values_videos; use explicit pooled_images"
        )
    ...
    def pool(self, frames):
        ...
        return normalize(
            np.mean(np.stack([self.image(f) for f in frames]), axis=0)[None]
        )[0]
