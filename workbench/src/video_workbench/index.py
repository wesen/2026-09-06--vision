...
def writer_lock(root):
    ...
    with (Path(root) / ".writer.lock").open("a") as lock:
        try:
            ...
            raise RuntimeError("another index writer is active")
        try:
            yield
        finally:
...
class FrameCache:
    ...
    def __init__(self, root, space, namespace="frames"):
        ...
        self.db.execute(
            "CREATE TABLE IF NOT EXISTS features (key TEXT PRIMARY KEY, sha256 TEXT NOT NULL)"
        )
        self.db.commit()
    ...
    def get(self, key):
        row = self.db.execute(
            "SELECT sha256 FROM features WHERE key=?", (key,)
        ).fetchone()
        if row is None:
            return None  # orphan arrays after a crash are not committed
        path = self.root / (key + ".npy")
        if file_hash(path) != row[0]:
            raise ValueError("corrupt cached feature")
        a = np.load(path, allow_pickle=False)
        if (
            a.shape != (1, self.space.dimension)
            or a.dtype != np.float32
            or not np.isfinite(a).all()
            or not np.allclose(np.linalg.norm(a, axis=1), 1, atol=1e-5)
        ):
    ...
    def put(self, key, vector, after_publish=None):
        ...
        if a.shape != (1, self.space.dimension):
        ...
        with self.db:
            self.db.execute(
                "INSERT OR REPLACE INTO features VALUES (?,?)", (key, file_hash(path))
            )
        return a[0]
...
def windows(duration_us, seconds):
    ...
    return [
        (start, min(start + size, duration_us)) for start in range(0, duration_us, size)
    ]
...
def frame_key(episode, index, space):
    ...
    return digest(
        {
            "space": space.id,
            "video": episode["video_sha256"],
            "raw_pts": m["raw_pts"][index],
            "time_base": m["time_base"],
            "origin_us": m["origin_us"],
            "pts_us": m["pts_us"][index],
        }
    )
    ...
    with writer_lock(root):
        ...
        try:
            ...
            for ep in episodes:
                ...
                pts = ep["media"]["pts_us"]
                plans = [
                    (a, b, selected_indices(pts, a, b, fps))
                    for a, b in windows(ep["media"]["duration_us"], seconds)
                ]
                required = sorted({i for _, _, ids in plans for i in ids})
                features = {
                    i: cache.get(frame_key(ep, i, embedder.space)) for i in required
                }
                missing = [i for i in required if features[i] is None]
                images = decode_selected(ep["video"], missing)
                for i in missing:
                    features[i] = cache.put(
                        frame_key(ep, i, embedder.space), embedder.image(images[i])
                    )
                    del images[i]
                fresh += len(missing)
                ...
                for start, end, ids in plans:
                    if not ids:
                        continue  # empty VFR spans contain no visual evidence
                    chunk = {
                    ...
                    chunks.append(chunk)
                    vectors.append(
                        normalize(np.mean([features[i] for i in ids], axis=0)[None])[0]
                    )
                progress(
                    f"{ep['episode_id']}: {len(plans)} windows, {len(missing)} new frames",
                    flush=True,
                )
            spec = {
                "schema": 1,
                ...
                "producer_files": {
                    ...
                    for name in ("embedding.py", "media.py", "index.py")
                },
                "corpus": [
                    {
                        k: e[k]
            ...
            if (directory / "manifest.json").exists():
                existing = Index(directory / "manifest.json", embedder.space.id)
                if not np.array_equal(existing.vectors, array):
                    raise ValueError("immutable index result changed")
            else:
...
class Index:
    def __init__(self, manifest, expected_space=None):
        ...
        if digest(m["space"]) != m["space_id"]:
            raise ValueError("invalid space identity")
        if expected_space is not None and m["space_id"] != expected_space:
            raise ValueError("incompatible feature space")
        spec = {
            k: v for k, v in m.items() if k not in ("index_id", "array_sha256", "shape")
        }
        if digest(spec) != m["index_id"]:
            raise ValueError("manifest identity mismatch")
        array = self.path.parent / "features.npy"
        if file_hash(array) != m["array_sha256"]:
            raise ValueError("index array hash mismatch")
        self.vectors = np.load(array, mmap_mode="r", allow_pickle=False)
        if (
            list(self.vectors.shape) != m["shape"]
            or self.vectors.shape != (len(m["chunks"]), m["space"]["dimension"])
            or self.vectors.dtype != np.float32
        ):
            raise ValueError("invalid index shape or dtype")
        if not np.isfinite(self.vectors).all() or not np.allclose(
            np.linalg.norm(self.vectors, axis=1), 1, atol=1e-5
        ):
            raise ValueError("invalid index normalization")

    def search(self, query, space_id, k=10, split=None):
        if space_id != self.manifest["space_id"]:
            raise ValueError("incompatible query feature space")
        if k < 1 or k > 100:
        ...
        ordered = sorted(
            eligible,
            key=lambda i: (-float(scores[i]), self.manifest["chunks"][i]["chunk_id"]),
        )[:k]
        return [
            dict(self.manifest["chunks"][i], score=float(scores[i]), rank=rank + 1)
