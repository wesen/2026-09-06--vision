...
def reasoning(root,request):
    ...
    o=request['options'];profile=make_profile(o['model'],reasoning=o['reasoning'])
    ...
    for f in request['evidence']['frames']:
        ...
        run(str(ROOT/MODELS[o['model']][2]),rp,out,'visibility',pp)
    ...
    return dict(kind=o['component'],mode='independent-single-image',profile=profile,profile_sha256=profile_hash(profile),
                checkpoint=str(ROOT/MODELS[o['model']][2]),records=records,events=transitions(records,int(o['max_gap_seconds']*1e6)))
...
def main():
    ...
    result=perception(root,request) if c in ('detection','segmentation','tracking') else reasoning(root,request) if c in ('reasoning','states') else embeddings(root,request)
