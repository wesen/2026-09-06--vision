...
def probe(path):
    with av.open(str(path)) as container:
        ...
        for frame in container.decode(stream):
            ...
            last_duration = micros(frame.duration * frame.time_base) if frame.duration else None
        ...
        tail = last_duration or (pts[-1]-pts[-2] if len(pts)>1 else
            micros(1/Fraction(stream.average_rate)) if stream.average_rate else 1)
        return {'pts_us': pts, 'raw_pts': raw, 'origin_us': origin,
            'time_base': str(stream.time_base), 'duration_us': pts[-1]+tail,
            'tail_duration_source': 'frame' if last_duration else 'estimated',
            'width': stream.width, 'height': stream.height,
            'frames': len(pts), 'variable_rate': len(set(b-a for a,b in zip(pts,pts[1:])))>1}
...
def decode_selected(path, indices):
    wanted=set(indices)
    images={}
    if not wanted:return images
    with av.open(str(path)) as container:
        for i, frame in enumerate(container.decode(video=0)):
            if i in wanted: images[i]=frame.to_image()
            if i >= max(wanted):break
    if set(images) != wanted:raise ValueError('video changed or selected frame missing')
    return images
