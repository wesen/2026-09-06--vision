...
class FakeEmbedder:
    ...
    def image(self,image):
        return normalize([[float(np.array(image).mean())/255,1,.2]])[0]


def test_orphan_recovery_and_corruption(tmp_path):
    cache=FrameCache(tmp_path,FakeEmbedder.space)
    def crash():raise RuntimeError('simulated crash after rename before transaction')
    with pytest.raises(RuntimeError):cache.put('frame',[1,2,3],after_publish=crash)
    assert cache.get('frame') is None
    cache.put('frame',[1,2,3]);assert cache.get('frame') is not None
    (cache.root/'frame.npy').write_bytes(b'bad')
    with pytest.raises(ValueError,match='corrupt'):cache.get('frame')
    cache.close()


def test_build_reuse_integrity_and_space(tmp_path,video):
    r=Registry(tmp_path/'r.sqlite');r.ingest(manifest(tmp_path,[row(video)]))
    e=FakeEmbedder();root=tmp_path/'cache'
    a=build(r,e,root,.2,5,progress=lambda *a,**k:None)
    b=build(r,e,root,.2,5,progress=lambda *a,**k:None)
    ...
    with pytest.raises(ValueError,match='incompatible'):Index(a['manifest'],'different')
    with pytest.raises(ValueError,match='incompatible'):idx.search([1,1,0],'different')
    ...
    p=idx.path.parent/'features.npy';p.write_bytes(b'corrupt')
    with pytest.raises(ValueError,match='hash'):Index(a['manifest'])
    r.close()
