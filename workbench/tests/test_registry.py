...
def video(tmp_path):
    ...
    with av.open(str(p),'w') as c:
        s=c.add_stream('libx264',rate=10);s.width=64;s.height=48;s.pix_fmt='yuv420p';s.time_base=Fraction(1,1000);s.codec_context.time_base=Fraction(1,1000)
        for i,pts in enumerate([0,100,350,450]):
            f=av.VideoFrame.from_ndarray(np.full((48,64,3),i*60,dtype=np.uint8),format='rgb24')
...
def row(path,**kwargs):
    return dict(episode_id='ep-one',split='train',split_group='g1',video=path.name,video_sha256=file_hash(path))|kwargs


def manifest(tmp_path, rows):
    p=tmp_path/'inputs.jsonl';p.write_text('\n'.join(json.dumps(r) for r in rows));return p
...
def test_ingest_repeat_and_rollback(tmp_path,video):
    ...
    with pytest.raises(ValueError,match='hash mismatch'):
        r.ingest(manifest(tmp_path,[row(video,episode_id='ep-two'),row(video,episode_id='ep-bad',video_sha256='bad')]))
...
def test_duplicate_and_corrupt(tmp_path,video):
    r=Registry(tmp_path/'r.sqlite')
    with pytest.raises(ValueError,match='duplicate'):r.ingest(manifest(tmp_path,[row(video),row(video)]))
    video.write_bytes(b'not video')
    with pytest.raises(av.error.InvalidDataError):r.ingest(manifest(tmp_path,[row(video)]))
    assert r.episodes()==[]
    r.close()
